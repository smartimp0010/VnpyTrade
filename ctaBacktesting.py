...
class BacktestingEngine(object):
    ...
    def loadHistoryDataCsv(self):
        ...
        df = pd.read_csv(
            'C:/Users/14123/Desktop/TXF01_Tick.csv', index_col=False)
    ...
    def plotResult(self):
        ...
        if self.plotMode == self.ORDER_MODE:
            left = pd.DataFrame({'Time': self.T, 'S': self.S})
            right = pd.DataFrame({'Time': self.OrderBuyTime, 'OrderBuy': self.OrderBuy})
            left = pd.merge(left, right, on=['Time'], how='left')
            right = pd.DataFrame({'Time': self.OrderSellTime, 'SellDeal': self.OrderSell})
            res = pd.merge(left, right, on=['Time'], how='left')
            pS.plot(res['Time'], res['S'], color='c', lw=0.8)
            pS.scatter(res['Time'], res['OrderBuy'], color='r')
            pS.scatter(res['Time'], res['SellDeal'], color='g', marker='*')
        else:
            left = pd.DataFrame({'Time': self.T, 'S': self.S})
            right = pd.DataFrame({'Time': self.BuyDealTime, 'OrderBuy': self.BuyDeal})
            left = pd.merge(left, right, on=['Time'], how='left')
            right = pd.DataFrame({'Time': self.SellDealTime, 'SellDeal': self.SellDeal})
            res = pd.merge(left, right, on=['Time'], how='left')
            pS.plot(res['Time'], res['S'], color='c', lw=0.8)
            pS.scatter(res['Time'], res['BuyDeal'], color='r')
            pS.scatter(res['Time'], res['SellDeal'], color='g', marker='*')
